...
public:
    ...
    Library* library;
    int number;
    Queue <int, 10> queueReades;

    Reader(string f, int n, Librarian* lib, Library* libb, int num) : People(f, n), librarian(lib), library(libb), number(num)
    {
    }
    ...
    void takeBook(Book book) override
    {
        ...
        queueReades.push(number);//add number readersa
    ...
    void giveBook(Book book) override
    {
        ...
        queueReades.pop();//delete number readers   
